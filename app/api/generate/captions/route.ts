...
export async function POST(request: Request) {
  ...
  try {
    ...
    const generated = await generateScreenshotCaptions({
      project: {
    ...
    const charged = await createVersionedGenerationResultAndDebitCredits({
      ...
      type: AssetType.SCREENSHOT_CAPTIONS,
      locale: project.primaryLanguage,
      prompt: generated.prompt,
      model: generated.model,
      content: generated.content,
      creditsCost: getAssetGenerationCreditCost(AssetType.SCREENSHOT_CAPTIONS),
      ledgerDescription: "Generated screenshot captions",
    });
    ...
    await writeUsageLog({
      userId: user.id,
      projectId: project.id,
      action: "generate_screenshot_captions",
      status: "success",
      model: generated.model,
    ...
    if (error instanceof OpenRouterServiceError) {
      return NextResponse.json(
        {
          error: "Screenshot captions generation failed",
